...
import { SafeAreaView, RoundedScrollContainer, ButtonContainer } from '@components/containers'
...
const PrivacyPolicy = ({ navigation, route }) => {
    ...
    const { updateCheckedState } = route.params;
    return (
        <SafeAreaView>
            <NavigationHeader
...
1                    1	  SALES IN CHARGE{'\n'}
                ...
                <View style={{ flexDirection: 'row', flex: 1, justifyContent: 'space-between' }}>
                    <View style={{ flex: 2 }}>
                        <Button title={'ACCEPT'}  onPress={() => { if (updateCheckedState) { updateCheckedState(true) } navigation.goBack() }}  />
                    </View>
                    <View style={{ width: 10 }} />
                    <View style={{ flex: 2 }}>
                        <Button title={'REJECT'} onPress={() => { if (updateCheckedState) { updateCheckedState(false) } navigation.goBack() }} />
                    </View>
                </View>
            </RoundedScrollContainer>
        </SafeAreaView>
